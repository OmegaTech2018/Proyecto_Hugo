		...
		<section id="cuerpo">
			...
			<?php 
				if($num_paciente>0){
    				...
    				$sql_datos_paciente="SELECT paciente_id, CONCAT(apellido_paterno,' ',apellido_materno,' ',nombre) AS nombre_paciente FROM paciente WHERE apellido_paterno LIKE '%$busca_paciente%' AND paciente_activo=1;";
    				if($result_datos_paciente=$sistema->enlaceBD->query($sql_datos_paciente)){
    					while($row_datos_paciente=$result_datos_paciente->fetch_object()){
    						...
    						echo "<a href=\"abrir_expediente.php?paciente_id=$paciente_id&nombre_paciente=$nombre_paciente&opcion=$opcion\">$nombre_paciente</a><br>";
    					}
    				}
    			}else{
