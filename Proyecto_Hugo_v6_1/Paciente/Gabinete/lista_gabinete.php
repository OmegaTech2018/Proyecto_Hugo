				...
				switch ($gabinete) {
					...
					case 1:
						...
						if($num_electro>0){
    						...
    						$sql_datos_electro="SELECT n.nota_id,n.fecha_nota,ge.gabinete_electrocardiogramas_id,descripcion_diagnostico 
												FROM paciente p INNER JOIN nota n ON p.paciente_id=n.paciente_id
												INNER JOIN gabinete_electrocardiogramas ge ON n.nota_id=ge.nota_id
    						...
    						if($result_datos_electro=$sistema->enlaceBD->query($sql_datos_electro)){
    							while($row_datos_electro=$result_datos_electro->fetch_object()){
    								...
    								$descripcion_diagnostico=$row_datos_electro->descripcion_diagnostico;
    								echo "<a href=\"abrir_gabinete.php?electro_id=$electro_id&Nombre_paciente=$Nombre_paciente&paciente_id=$paciente_id&gabinete=$gabinete&nota_id=$nota_id&descripcion_diagnostico=$descripcion_diagnostico\">
    								    Fecha Nota: ".date('d-m-Y',strtotime($fecha_nota))." Diagnostico: $descripcion_diagnostico</a><br>";
    							}
    						}
    					}else{
					...
					case 2:
						...
						if($num_papanicolaou>0){
							//echo $num_paciente."<br>";
    						$sql_datos_papanicolaou="SELECT n.nota_id,n.fecha_nota,gp.gabinete_papanicolaou_id,descripcion_diagnostico 
													 FROM paciente p INNER JOIN nota n ON p.paciente_id=n.paciente_id
													 INNER JOIN gabinete_papanicolaou gp ON n.nota_id=gp.nota_id
													 INNER JOIN diagnostico d ON n.diagnostico_id=d.diagnostico_id
													 WHERE p.paciente_id=$paciente_id AND p.paciente_activo=1 AND gp.bandera=1;";
    						if($result_datos_papanicolaou=$sistema->enlaceBD->query($sql_datos_papanicolaou)){
    							while($row_datos_papanicolaou=$result_datos_papanicolaou->fetch_object()){
    								...
    								$descripcion_diagnostico=$row_datos_papanicolaou->descripcion_diagnostico;
    								echo "<a href=\"abrir_gabinete.php?papanicolaou_id=$papanicolaou_id&Nombre_paciente=$Nombre_paciente&paciente_id=$paciente_id&gabinete=$gabinete&nota_id=$nota_id&descripcion_diagnostico=$descripcion_diagnostico\">
    								    Fecha Nota: ".date('d-m-Y',strtotime($fecha_nota))." Diagnostico: $descripcion_diagnostico</a><br>";
    							}
    						}
    					}else{
					...
					case 3:
						...
						if($num_ultrasonidos>0){
							//echo $num_paciente."<br>";
    						$sql_datos_ultrasonido="SELECT n.nota_id,n.fecha_nota,gu.gabinete_ultrasonidos_id,descripcion_diagnostico
													FROM paciente p INNER JOIN nota n ON p.paciente_id=n.paciente_id
													INNER JOIN gabinete_ultrasonidos gu ON n.nota_id=gu.nota_id
													INNER JOIN diagnostico d ON n.diagnostico_id=d.diagnostico_id
													WHERE p.paciente_id=$paciente_id AND p.paciente_activo=1 AND gu.bandera=1;";
    						if($result_datos_ultrasonido=$sistema->enlaceBD->query($sql_datos_ultrasonido)){
    							while($row_datos_ultrasonido=$result_datos_ultrasonido->fetch_object()){
    								...
    								$descripcion_diagnostico=$row_datos_ultrasonido->descripcion_diagnostico;
    								echo "<a href=\"abrir_gabinete.php?ultrasonidos_id=$ultrasonidos_id&Nombre_paciente=$Nombre_paciente&paciente_id=$paciente_id&gabinete=$gabinete&nota_id=$nota_id&descripcion_diagnostico=$descripcion_diagnostico\">
    									Fecha Nota: ".date('d-m-Y',strtotime($fecha_nota))." Diagnostico: $descripcion_diagnostico</a><br>";
    							}
    						}
    					}else{
					...
					case 4:
						...
						if($num_rayosx>0){
							//echo $num_paciente."<br>";
    						$sql_datos_rayosx="SELECT n.nota_id,n.fecha_nota,gr.gabinete_rayosx_id,descripcion_diagnostico
											   FROM paciente p INNER JOIN nota n ON p.paciente_id=n.paciente_id
											   INNER JOIN gabinete_rayosx gr ON n.nota_id=gr.nota_id
											   INNER JOIN diagnostico d ON n.diagnostico_id=d.diagnostico_id
											   WHERE p.paciente_id=$paciente_id AND p.paciente_activo=1 AND gr.bandera=1;";
    						if($result_datos_rayosx=$sistema->enlaceBD->query($sql_datos_rayosx)){
    							while($row_datos_rayosx=$result_datos_rayosx->fetch_object()){
    								...
    								$descripcion_diagnostico=$row_datos_rayosx->descripcion_diagnostico;
    								echo "<a href=\"abrir_gabinete.php?rayosx_id=$rayosx_id&Nombre_paciente=$Nombre_paciente&paciente_id=$paciente_id&gabinete=$gabinete&nota_id=$nota_id&descripcion_diagnostico=$descripcion_diagnostico\">
    									Fecha Nota: ".date('d-m-Y',strtotime($fecha_nota))." Diagnostico: $descripcion_diagnostico</a><br>";
    							}
    						}
    					}else{
